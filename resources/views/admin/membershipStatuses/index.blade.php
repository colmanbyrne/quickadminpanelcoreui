...
@can('membership_status_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.membership-statuses.create') }}">
                        ...
                        <tr data-entry-id="{{ $membershipStatus->id }}">
                            ...
                            <td>
                                @can('membership_status_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.membership-statuses.show', $membershipStatus->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('membership_status_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.membership-statuses.edit', $membershipStatus->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('membership_status_delete')
                                    <form action="{{ route('admin.membership-statuses.destroy', $membershipStatus->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('membership_status_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.membership-statuses.massDestroy') }}",
